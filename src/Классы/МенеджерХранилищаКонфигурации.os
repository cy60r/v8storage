...
#Область РаботаСВерсиямиХранилища

Функция ПолучитьТаблицуВерсийСВерсии(НомерНачальнойВерсии = 1) Экспорт

    ПроверитьЗагрузкуДанныхХранилищаСВерсии(НомерНачальнойВерсии);

    Возврат ТаблицаВерсий;

КонецФункции

Процедура ПроверитьЗагрузкуДанныхХранилищаСВерсии(НомерНачальнойВерсии)

    Если Не ЧтениеХранилищаВыполнено Тогда
       ПрочитатьХранилище(НомерНачальнойВерсии);
    КонецЕсли;
